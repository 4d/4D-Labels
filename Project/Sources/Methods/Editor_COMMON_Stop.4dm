...
#DECLARE($tool : Text; $mouse_x : Integer; $mouse_y : Integer) : Boolean
...
Else 
	...
	Case of 
		...
		: ($tool="polyline")
			...
			$Txt_data:=$Txt_data+" "+String:C10(($mouse_x-$Lon_xClick)/$Num_zoom; "&xml")+","+String:C10(($mouse_y-$Lon_yClick)/$Num_zoom; "&xml")
		...
		Else 
			...
			$Lon_width:=Abs:C99($mouse_x-$Lon_xClick)
			$Lon_height:=Abs:C99($mouse_y-$Lon_yClick)
			
			$Num_xTranslation:=Choose:C955($mouse_x>$Lon_xClick; ($Lon_width/2)+$Lon_xClick; ($Lon_width/2)+$mouse_x)
			$Num_yTranslation:=Choose:C955($mouse_y>$Lon_yClick; ($Lon_height/2)+$Lon_yClick; ($Lon_height/2)+$mouse_y)
	...
	Case of 
		...
		: ($tool="rect")\
			...
			DOM SET XML ATTRIBUTE:C866($Dom_object; \
				...
				"rx"; Choose:C955($tool="round-rect"; OB Get:C1224(<>label_params; "defaultRoundRect"; Is longint:K8:6); 0); \
				"ry"; Choose:C955($tool="round-rect"; OB Get:C1224(<>label_params; "defaultRoundRect"; Is longint:K8:6); 0))
		...
		: ($tool="line")
			...
			Case of 
				...
				: ($mouse_x<$Lon_xClick)\
					...
					//----------------------------------------
				: ($mouse_x=$Lon_xClick)  //vertical
					
					Case of 
							
							//……………………………………………………………
						: ($mouse_y<$Lon_yClick)  //bottom to top
							...
							//……………………………………………………………
						: ($mouse_y>$Lon_yClick)  //top to bottom
				...
				: ($mouse_y=$Lon_yClick)  //horizontal
					...
					Case of 
							...
							//……………………………………………………………
						: ($mouse_x<$Lon_xClick)  //right to left
							
							$Lon_x2:=-($Lon_width/2)
							$Lon_x1:=-$Lon_x2
							
							//……………………………………………………………
						: ($mouse_x>$Lon_xClick)  //left to right
		...
		Else 
			...
			ASSERT:C1129(False:C215; "Unknown entry point: \""+$tool+"\"")
